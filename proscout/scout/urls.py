...
from . import views
from scout.api import ProfileView, UpdateProfile


urlpatterns = [
    url('index', views.index, name='homepage'),
    re_path('profile/(?P<username>.+)/$', ProfileView.as_view()),
    url('profile/edit/(?P<username>.+)/$', UpdateProfile.as_view(), name='edit_profile'),
